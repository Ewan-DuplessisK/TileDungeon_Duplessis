...
void Renderer::drawSprite(const Actor& actor, const Texture& tex, Rectangle scRect, Vector2 origin, Flip flip) const{
	SDL_Rect dstRect;
	...
	if (scRect != Rectangle::nullRect){
		srcSDL = new SDL_Rect{
		Maths::round(scRect.x),
		Maths::round(scRect.y),
		Maths::round(scRect.width),
		Maths::round(scRect.height)
		};
	}
	...
	SDL_RenderCopyEx(
		...
		nullptr,
		// rotation point, center by default
		SDL_FLIP_NONE);
